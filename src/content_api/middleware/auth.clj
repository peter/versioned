(ns content-api.middleware.auth
  (:require [content-api.util.auth :refer [parse-token]]
            [content-api.logger :as logger]
            [content-api.models.users :as users]))
...
(defn require-auth [request handler app]
  (let [access-token (parse-token (:headers request))
    ...
    (if (and access-token user (not (users/token-expired? user (:config app))))
      (do (logger/debug app "content-api.middleware.auth/require-auth success user: " user)
          (handler (assoc request :user user)))
      (do (logger/debug app "content-api.middleware.auth/require-auth failure user: " user " token: " access-token)
