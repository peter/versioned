(ns content-api.model-relationships
  (:require [content-api.model-support :as model-support]
            [content-api.model-versions :refer [apply-version versioned-coll published-model?]]
...
(defn find-relationship-to-many [app model-spec doc relationship opts]
  (let [spec (get-in model-spec [:relationships (keyword relationship)])
        ...
        ordered-docs (map #(u/find-property docs :id %) ids)] ; FIXME: too slow?
