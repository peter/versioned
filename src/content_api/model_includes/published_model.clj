(ns content-api.model-includes.published-model
  (:require [content-api.model-includes.versioned-model :refer [latest-version]]))
...
(def published-schema {
  :type "object"
  :properties {
    :published_version {:type "integer" :minimum 1}
    :publish_at {:type "string" :format "date-time"}
    :unpublish_at {:type "string" :format "date-time"}
  }
})
