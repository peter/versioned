(ns content-api.routes
  (:require [content-api.util.core :as u]
            [content-api.crud-api :as crud-api]))
...
(defn- lookup-var-handler [controller action]
  (let [path (str "content-api.controllers." controller "/" action)]
    (u/load-var path)))
...
(defn- lookup-api-handler [models controller action]
  (let [crud-fn (u/load-var (str "content-api.crud-api/" action))
